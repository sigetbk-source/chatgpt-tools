> [!NOTE]
> このディレクトリは、`sigetbk-source/ticket-launcher` のローカル開発ブランチ `codex/mvp-implementation`、コミット `fe7231f8b4d7ac0d3297ca4ffa6add1f2cbad9a7` を 2026-08-23 にコピーして集約したものです。基点となる GitHub `main` は `8513697bc9bfc28cd03f65eaa3e6c3bcacd11818` です。元リポジトリは削除・アーカイブせず、変更せずに残しています。履歴の完全移植ではなく、現行のコミット済みファイル一式を優先したスナップショットです。コピー時にローカル作業ツリーにあった未コミットの `project.pbxproj` 変更（2行）は含めていません。
